<template>
  <v-dialog v-model="dialog" width="unset">
    ...
    <v-btn rounded class="mx-4" color="primary" v-on="on">
     <!-- <v-btn @click="login" size="20px" rounded color="tea1 accent-3" dark>SIGN IN </v-btn> -->
        <!-- <v-icon size="20px">fas fa-cut</v-icon> -->
    </v-btn>
    </template>
    <v-card>
    <!-- <v-card-title>{{article.articleid}}</v-card-title> -->
    <v-card-text class="">로그인 성공입니다!</v-card-text>
    <v-card-actions>
        <v-spacer></v-spacer>
        <!-- <v-btn color="green darken-1" text @click="dialog = false">Disagree</v-btn> -->
        <v-btn color="green darken-1" text @click="goHome">Agree</v-btn>
    </v-card-actions>
    </v-card>
  </v-dialog>
</template>
